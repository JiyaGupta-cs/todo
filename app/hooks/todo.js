...
//                 setTransactionPending(true)
//                 const[profilePda,profileBump] = findProgramAddressSync([utf8.encode('USER_STATE'),publicKey.toBuffer()],program.programId);
//                 const [todoPda, todoBump]= findProgramAddressSync([utf8.encode('TODO_STATE'),publicKey.toBuffer(), Uint8Array([lastTodo])],program.programId);
...
//     return { initialized, initializeStaticUser, loading, transactionPending, completedTodos, incompleteTodos, markStaticTodo, removeStaticTodo, addTodo, input, setInput, handleChange , initializeUser}
// }
...
export function useTodo() {
    ...
    useEffect(() => {
        if (program && publicKey && !transactionPending) {
            const findProfileAccounts = async () => {
                try {
                    ...
                    if (profileAccount) {
                        ...
                        setInitialized(true)
                        if (profileAccount.todoCount > 0) {
                            const [todoPda] = findProgramAddressSync([utf8.encode('TODO_STATE'), publicKey.toBuffer(), Uint8Array.from([lastTodo])], program.programId)
                            const todoAccounts = await program.account.toDoAccount.all([authorFilter(publicKey.toString())])
                            setTodos(todoAccounts)
                        }
                    } else {
            ...
            // Run the profile fetching function inside useEffect
            findProfileAccounts()
        }
    }, [publicKey, program, transactionPending, lastTodo])
